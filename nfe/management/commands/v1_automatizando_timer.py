...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        while True:  # Loop infinito
            ...
            self.stdout.write(f"[{agora}] Iniciando nova iteração de consulta...")

            try:
                token = self.obter_token()
            except RuntimeError as e:
                self.stderr.write(str(e))
                self.stdout.write("[INFO] Aguardando 1 hora para próxima tentativa.")
                time.sleep(3600)
                continue
            ...
            for empresa in Empresa.objects.all():
                ...
                try:
                    ...
                    if not response or not response.text.startswith('<'):
                        print(f"[{agora}] Erro: Resposta inválida ou não-XML.")
                        print(f"[{agora}] Resposta completa:\n{response.text}")
                        continue
                    ...
                    for doc in documentos:
                        ...
                        try:
                            data = {
                                ...
                                "nsu": numero_nsu,
                                "xml": conteudo,
                                "fileXml": relative_path,
                            }
                            api_url = f"{settings.API_URL}/nfes/"
            ...
            self.stdout.write(f"[{agora}] Iteração finalizada. Aguardando 1 hora para a próxima execução...")
            time.sleep(3600)
